drop function if exists public.send_bulk_poke_messages(uuid[], text, timestamptz);
...
begin
  ...
  v_message_text := trim(coalesce(p_message_text, ''));
...
grant all on function public.send_bulk_poke_messages(uuid[], text) to authenticated;
